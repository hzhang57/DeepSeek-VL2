...
def main(args):
    ...
    qa1_acc = TypeAccuracy("Causal")
    qa2_acc = TypeAccuracy("Temporal")
    qa3_acc = TypeAccuracy("Descriptive")
    ...
    for line in tqdm(annotations, total=len(annotations)):
        ...
        candidates = [ key for key in index2ans.keys() ]
        answer_id =  random.choice(candidates)
        global_acc.update(gt_answers, answer_id)
        print("{}:\n{}".format(idx, qs))
        #print("Global Accu{:.4f}.\nGT: {}\nAI: {}".format(correct*1.0/total, gt_answers, outputs))
        print("GT: {}\nAI: {}".format(gt_answers, answer_id))
        if "C" == quest_type[0]:
        ...
        print("-----"*5)
        # average over type
        avg_acc = (qa1_acc.get_accuracy() + qa2_acc.get_accuracy() + qa3_acc.get_accuracy() ) / 3.0
