...
def parse_choice(response, all_choices, index2ans=None):
    ...
    if len(candidates) == 0:
        for choice in all_choices:  # e.g., A B C D
            check = choice.replace("(", "").replace("(", "")  # e.g., A B C D
            if f" {check} " in response:
                candidates.append(choice)

    # if all above doesn't get candidates, check if the content is larger than 5 tokens and try to parse the example
    #if len(candidates) == 0 and len(response.split()) > 2 and index2ans is not None:
    if len(candidates) == 0 and index2ans is not None:
        for index, ans in index2ans.items():
            if ans.lower() in response.lower():
                candidates.append(index)
                index_ans = False  # it's content ans.
            elif response.lower().strip(".") in ans.lower():
                candidates.append(index)
                index_ans = False  # it's content ans.
